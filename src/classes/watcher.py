...
class Watcher:
    ...
    def run(self):
        ...
        self.entries = self.find_new_entries(start_date) # Find the new entries since this date
    ...
    def save_run(self, run: WatcherRun):
        ...
        if len(self.entries) == 0 and not self.save_if_empty:
            logging.info(f"Watcher {self.name} has no new entries and will not save the run")
            return
